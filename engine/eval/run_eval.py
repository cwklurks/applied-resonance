...
def render_report(
    *,
    ...
    lines.append(
        ...
        "(subset: 35 normal + 15 abnormal per id)"
    )
    lines.append(
    ...
    if classifier_metrics is None:
        ...
        if m["val_acc"] < 0.85:
            lines.append(
                ...
                "per machine on this 400-clip subset, the CNN latches onto "
                "id-specific spectral signatures that do not transfer to the "
                "held-out id_06 hardware (the fan/pump log-mel mean ordering "
                "even inverts on id_06), so it collapses toward predicting one "
                "class. Expect this to recover on the full dataset (more ids "
                "and clips); the anomaly scorers above are unaffected since "
                "they are fit per-id."
            )
    lines.append("")
    ...
    lines.append(
        f"| this run (kNN, subset) | {fan_str} | {pump_str} | {verdict} |"
    )
    lines.append("")
    lines.append(
        "This subset has only 15 abnormal + 15 test-normal clips per id, so each "
        "per-id AUC has wide confidence intervals; treat the type averages as "
        "indicative, not precise. To run on the full dataset, download it with "
        "`uv run python -m engine.download --snr 0 --machines fan pump` and then "
        "rerun this eval."
    )
    lines.append("")
