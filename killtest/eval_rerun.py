...
def rerun_eval(
    ...
    for split in splits:
        ...
        test_clips.extend(split.test_normal)
        test_clips.extend(split.test_abnormal)
...
def render_report(data: dict) -> str:
    ...
    lines.append("")
    lines.append(
        "| condition | n_replaced | fan kNN AUC | pump kNN AUC | "
...
def _print_summary(label: str, result: dict, report_path: Path) -> None:
    print(f"\n=== eval_rerun: {label} ===")
    print(f"  test clips: {result['n_test']}  replaced: {result['n_replaced']}")
